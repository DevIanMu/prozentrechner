...
function buildInitialValues(mode: CalculatorMode): Record<string, number | null> {
  return Object.fromEntries(
    mode.inputFields.map((field) => [field.name, null])
  );
}

export function CalculatorClient({ mode, content }: CalculatorClientProps) {
  const [isExplanationExpanded, setIsExplanationExpanded] = React.useState(false);
  const initialValues = React.useMemo(() => buildInitialValues(mode), [mode]);
